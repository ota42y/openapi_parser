...
RSpec.describe 'OpenAPIParser::SpecValidator::Rules::ContentSchemaIn30' do
  ...
  def doc_with_content_schema(openapi_version_string)
    raw = base_doc(
  ...
  def doc_without_content_schema(openapi_version_string)
    raw = base_doc(openapi_version_string, { 'type' => 'string' })
    OpenAPIParser.parse(raw, strict_reference_validation: false)
  end

  def run_rule_for(root)
    OpenAPIParser::SpecValidator::Rules::ContentSchemaIn30.new(root.openapi_version).check(root)
  end
